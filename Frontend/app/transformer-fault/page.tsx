...
export default function TransformerFault() {
  ...
  const handleTrainLocalFaultModel = async () => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:8000/train_local_fault_model", formData, {
  ...
  const handleGetFaultPredictions = async () => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:8000/predict_fault", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
